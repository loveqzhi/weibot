...
class Connector extends Test {
	...
	public function testServerCookieSet() {
		$this->sendRequest();
		$this->assertTrue(isset($this->response->cookies[\PhpConsole\Connector::SERVER_COOKIE]));
		$this->assertEquals(\PhpConsole\Connector::SERVER_PROTOCOL, $this->response->cookies[\PhpConsole\Connector::SERVER_COOKIE]);
	}

	public static function provideDifferentProtocolVersions() {
		return static::getOneArgProviderData(array(
			\PhpConsole\Connector::SERVER_PROTOCOL - 1,
			\PhpConsole\Connector::SERVER_PROTOCOL + 1,
			'x'
		));
	}
	...
	public function testGetBackDataInResponse() {
		$data = array(1, 2, 3);
		$this->request->postData[\PhpConsole\Connector::POST_VAR_NAME]['getBackData'] = $data;
		$this->sendRequest();
